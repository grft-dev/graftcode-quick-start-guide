...
That is the main value of Graftcode: your JavaScript logic becomes a typed dependency for other applications, without writing REST handlers, request models, or client SDK code. If your service already has a REST API, it can stay intact while Graftcode Gateway adds this new typed access layer on top. By using Graftcode Gateway, you also expose MCP for AI scenarios. When you add or update a public method, consumers get notified through their package manager and can update with a single command - the same way they update any other dependency.
